    ...
    const getKey = (edge: ServidumbreEdge): string => {
        ...
        switch (edge.frontierType) {
            case 'internal':
                ...
                break
            case 'neighbor':
                identity = edge.neighbors.map(n => `${n.is_partial ? 'p:' : ''}${n.name}`).sort().join('+')
                break
            case 'external':
...
function renderGroupedBoundaries(groups: GroupedBoundary[]): string {
    const parts = groups.map(g => {
        ...
        if (uniqueColindas.size === 1) {
            ...
            const combinedDist = distTexts.length === 2
                ? `${distTexts[0]}, y en ${distTexts[1]}`
                : distTexts.join(', y en ')
        ...
        const segStrs = g.segments.map(seg => {
            const distText = seg.distance > 0
                ? numberToWordsLower(seg.distance) + ' metros'
                : BLANK
...
export function generateServidumbreText(
    ...
    const areaText = analysis.areaM2 > 0
        ? numberToWordsLower(analysis.areaM2)
        : BLANK
