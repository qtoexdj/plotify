...
function formatGroupedBoundaries(boundaries: OfficialBoundary[]): string[] {
    console.log('[DEBUG-GENERATOR] Iniciando formatGroupedBoundaries con:', JSON.stringify(boundaries, null, 2))
    ...
    return groups.map(g => {
        const tramosStrs = g.items.map(b => {
            const distance = b.distance != null && b.distance > 0
                ? numberToWordsLower(b.distance) + ' metros'
                : BLANK
...
export function generateDeslindeText(lot: DeslindeInput): string {
    ...
    const area = (lot.area_official_m2 && lot.area_official_m2 > 0)
        ? lot.area_official_m2
        : lot.m2
    const areaText = area != null && area > 0
        ? numberToWords(area)
        : BLANK

    // Servidumbre
    const servidumbreText = lot.servidumbre_m2 != null && lot.servidumbre_m2 > 0
        ? numberToWords(lot.servidumbre_m2)
        : BLANK
