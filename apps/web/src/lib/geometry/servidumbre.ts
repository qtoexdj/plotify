...
import type { ServidumbreEdge, ServidumbreTramo, ServidumbreAnalysis, ServidumbreFrontierType, NeighborMetadata } from '@/types/database.types'
 ...
 */
function toTurfFeature(geometry: GeoJSONGeometry): Feature<Polygon | MultiPolygon | LineString | MultiLineString> | null {
...
function groupEdgesIntoTramos(edges: ServidumbreEdge[]): ServidumbreTramo[] {
    ...
    let currentGroup = { direction: internalDirections[0].direction, startIdx: internalDirections[0].idx, endIdx: internalDirections[0].idx }
