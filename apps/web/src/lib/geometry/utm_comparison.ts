...
function compare(coords: number[][], name: string) {
  ...
  for (let i = 0; i < coords.length - 1; i++) {
    // assume closed or handle closing
  ...
  console.log(
    `Coordinates Sample: [${coords[0][0]}, ${coords[0][1]}] -> UTM Zone ${zone} [${utmCoords[0].x.toFixed(2)}, ${utmCoords[0].y.toFixed(2)}]`
  )
  ...
  console.log(`|-----------|--------------------|---------------|-----------|---------|`)
  console.log(
    `| Area      | ${geoArea.toFixed(4)} m²   | ${utmArea.toFixed(4)} m²| ${areaDiff.toFixed(4)} | ${areaDiffPct.toFixed(4)}% |`
  )
  console.log(
    `| Perimeter | ${geoPerim.toFixed(4)} m      | ${utmPerim.toFixed(4)} m     | ${perimDiff.toFixed(4)} | ${perimDiffPct.toFixed(4)}% |`
  )
  ...
  if (Math.abs(areaDiffPct) > 0.1) {
    console.log(
      `⚠️  Significant Area Discrepancy (>0.1%). likely due to UTM Scale Factor Distortion.`
    )
    console.log(
      `    UTM projects the curved earth onto a flat cylinder. Distances are scaled by factor K.`
    )
    ...
    console.log(
      `    SAG/CBR typically mandate UTM Planarian Area, effectively "forcing" the distortion error.`
    )
  } else {
