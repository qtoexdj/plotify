...
type RouteParams = { params: Promise<{ templateId: string; clauseKey: string }> }

export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    const { templateId, clauseKey } = await params
    ...
    const { data, error, status } = await microserviceFetch<EscrituraTemplateDetail>(
      `/api/v1/escritura-templates/${encodeURIComponent(templateId)}/clauses/${encodeURIComponent(clauseKey)}?${upstreamParams.toString()}`,
      {
        method: 'PUT',
        body,
      }
    ...
    console.error('Error in PUT /api/escritura-templates/[templateId]/clauses/[clauseKey]:', error)
