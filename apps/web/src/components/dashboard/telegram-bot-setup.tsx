...
import { BotIcon, CheckCircle2, ChevronRight, CopyIcon, LinkIcon, Loader2, Trash2, WebhookIcon } from "lucide-react"
...
export function TelegramBotSetup({ organizationId, onBotStatusChange }: { organizationId: string, onBotStatusChange?: (hasBot: boolean) => void }) {
    ...
    return (
      <Card className="border-primary/20 bg-primary/5">
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Tu organización está utilizando el bot <span className="font-semibold text-foreground">@{status.bot_username}</span>.
          </CardDescription>
        </CardHeader>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Se desconectará este bot y dejará de recibir notificaciones. 
                  Los usuarios tendrán que volver a vincular sus cuentas si configuras uno nuevo.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
    ...
    <div className="space-y-6">
      <div className="grid md:grid-cols-2 gap-6">
        <Card className="border-border shadow-sm">
          ...
          <CardContent className="space-y-4 text-sm text-muted-foreground">
            <ol className="list-decimal pl-4 space-y-2">
              <li>Abre Telegram y busca <strong className="text-foreground">@BotFather</strong></li>
              <li>Envíale el mensaje <code className="bg-muted px-1 py-0.5 rounded">/newbot</code></li>
              <li>Elige un <strong>nombre</strong> para tu bot (Ej: <em>Inmobiliaria Plotify Bot</em>)</li>
              <li>Elige un <strong>username</strong> que termine en &quot;bot&quot; (Ej: <em>MiPlotifyBot</em>)</li>
              <li>Copia el <strong>API Token</strong> que te entregará.</li>
            </ol>
            <Button variant="outline" className="w-full mt-2" onClick={() => window.open('https://t.me/BotFather', '_blank')}>
              Ir a @BotFather
        ...
        <Card className="border-primary/50 shadow-md ring-1 ring-primary/20 bg-background/50 relative overflow-hidden">
          ...
          <CardHeader>
            <div className="flex items-center gap-2 mb-1">
              <div className="flex items-center justify-center w-6 h-6 rounded-full bg-primary text-primary-foreground text-xs font-bold shadow-[0_0_10px_rgba(var(--primary),0.5)]">2</div>
