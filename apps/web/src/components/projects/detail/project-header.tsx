...
import { ArrowLeft01Icon, Location01Icon, PencilEdit02Icon, Delete02Icon } from '@hugeicons/core-free-icons'
...
export function ProjectHeader({ project, onDelete, isDeleting, isAdmin, children }: ProjectHeaderProps) {
    ...
    return (
        <div className="flex flex-col gap-3 md:flex-row md:flex-wrap md:items-center md:justify-between">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h1 className="text-xl md:text-3xl font-bold text-slate-900 dark:text-slate-100 leading-tight">{project.name}</h1>
                    <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                        <span className="flex items-center gap-1 text-sm text-slate-600 dark:text-slate-400">
                            <HugeiconsIcon icon={Location01Icon} className="w-3.5 h-3.5 shrink-0" />
                            {project.region} / {project.comuna}
                        </span>
                        <Badge variant="secondary" className="text-xs">{project.estado}</Badge>
                    </div>
                </div>
            </div>
