...
export function LegalTab({ lots, projectId, project }: LegalTabProps) {
    ...
    // Cache de análisis de servidumbre por lote
    const analysisCache = useRef<Map<string, ServidumbreAnalysis | null>>(new Map())
        ...
        const fetchFC = async () => {
            ...
            try {
                ...
                if (res.ok && !cancelled) {
                    ...
                    // Limpiar cache al recibir nuevas geometrías
    ...
    const servidumbreAnalysis = useMemo<ServidumbreAnalysis | null>(() => {
        ...
        // Revisar cache
        if (analysisCache.current.has(selectedLot.id)) {
            return analysisCache.current.get(selectedLot.id) ?? null
        }
        ...
        if (!lotFeature) {
            console.warn('[DEBUG-SERVIDUMBRE] No se encontró geometría para lote', selectedLot.numero_lote)
            analysisCache.current.set(selectedLot.id, null)
            return null
        }
        ...
        // Cachear resultado
        analysisCache.current.set(selectedLot.id, analysis)
        return analysis
