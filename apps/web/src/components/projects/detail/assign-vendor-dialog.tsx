...
import { getOrganizationMembersAction, assignVendorsToProjectAction } from '@/actions/vendor-actions.action'
...
export function AssignVendorDialog({
    ...
    const loadMembers = useCallback(async () => {
        ...
        if (result.success && result.data) {
            ...
            setMembers(result.data as OrganizationMemberWithProfile[])
        } else {
    ...
    // Sincronizar selección inicial basada en IDs de vendedores ya asignados
    useEffect(() => {
        if (members.length > 0 && assignedVendorIds.length > 0) {
