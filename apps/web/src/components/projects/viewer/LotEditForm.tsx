...
export function LotEditForm({ lotDetails, onSave, onCancel }: LotEditFormProps) {
    ...
    const currentEstado = watch('estado') as EstadoLote
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
            ...
            <div className="relative overflow-hidden rounded-xl bg-linear-to-br from-primary/5 via-sidebar-accent/50 to-transparent p-4 text-center border border-sidebar-border/50">
                <p className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/70">Editando Lote</p>
                ...
                {errors.numero_lote && <p className="text-[10px] text-red-500 mt-1">{errors.numero_lote.message}</p>}
            ...
            <Card className="shadow-xs border-sidebar-border/50">
                <CardContent className="p-4 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="space-y-2">
                            <Label className="text-xs font-medium text-sidebar-foreground/70">Valor Reserva ($)</Label>
                            <Input
                                type="number"
                            ...
                            />
                            {errors.valor_reserva && <p className="text-[10px] text-red-500">{errors.valor_reserva.message}</p>}
                        </div>
                    </div>
                    ...
                    <div className="flex gap-2 pt-2 border-t mt-2">
                        <Button type="submit" disabled={isSaving} className="flex-1 h-9" size="sm">
                            {isSaving ? <HugeiconsIcon icon={Loading02Icon} className="w-4 h-4 animate-spin mr-2" /> : <HugeiconsIcon icon={FloppyDiskIcon} className="w-4 h-4 mr-2" />}
                            Guardar Cambios
                        </Button>
                        <Button
                            type="button"
