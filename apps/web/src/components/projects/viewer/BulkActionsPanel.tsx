...
import { Loading02Icon, Tick02Icon, Cancel01Icon, Money01Icon, Layers01Icon } from '@hugeicons/core-free-icons'
            ...
            <div className="bg-primary/5 rounded-xl p-4 border border-primary/10">
                ...
                <div className="grid grid-cols-2 gap-2 mt-3">
                    <div className="bg-card/50 rounded-lg p-2 text-center border shadow-xs">
                        <p className="text-[10px] font-semibold uppercase text-muted-foreground/70">Sup. Total</p>
                        <p className="font-mono text-sm font-bold text-foreground">
                            {new Intl.NumberFormat('es-CL').format(totalArea)} m²
                        </p>
                    </div>
                    <div className="bg-card/50 rounded-lg p-2 text-center border shadow-xs">
                        <p className="text-[10px] font-semibold uppercase text-muted-foreground/70">Valor Total</p>
                        <p className="font-mono text-sm font-bold text-foreground">
                            {totalPrice > 0
                                ? new Intl.NumberFormat('es-CL', { notation: 'compact', compactDisplay: 'short' }).format(totalPrice)
                                : '--'}
                        </p>
                    </div>
                </div>
            ...
            <div className="space-y-4 shrink-0">
                ...
                <Card className="shadow-none border-dashed bg-muted/20">
                    ...
                    <CardContent className="p-3 pt-2 flex gap-2">
                        ...
                        >
                            {isUpdatingState ? <HugeiconsIcon icon={Loading02Icon} className="w-3 h-3 animate-spin" /> : 'Aplicar'}
                        </Button>
                    </CardContent>
                </Card>
                ...
                <Card className="shadow-none border-dashed bg-muted/20">
                    <CardHeader className="p-3 pb-1">
                        ...
                        <CardDescription className="text-[10px]">Asigna el mismo precio a todos</CardDescription>
                    </CardHeader>
                    <CardContent className="p-3 pt-2 flex gap-2">
                        <Input
                            type="number"
                        ...
                        >
                            {isUpdatingPrice ? <HugeiconsIcon icon={Loading02Icon} className="w-3 h-3 animate-spin" /> : 'Aplicar'}
                        </Button>
                    </CardContent>
                </Card>
            </div>
                ...
                <ScrollArea className="h-full pr-3">
                    <div className="space-y-1.5">
                        {selectedFeatures.map(f => (
                            <div key={f.properties.geometry_id} className="flex items-center justify-between p-2 rounded-md bg-card border shadow-xs group hover:border-primary/30 transition-colors">
                                <div className="flex items-center gap-2">
                                    <Badge variant="outline" className="font-mono text-[10px] h-5 min-w-12 justify-center">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    >
                                        <HugeiconsIcon icon={Cancel01Icon} className="w-3 h-3 text-muted-foreground hover:text-destructive" />
                                    </Button>
                                </div>
                            </div>
                        ))}
