    ...
    return (
        <div className="space-y-3">
            ...
            <div className="relative overflow-hidden rounded-xl bg-linear-to-br from-primary/10 via-primary/5 to-transparent p-5 text-center border border-primary/10 shadow-xs">
                ...
                <p className="text-3xl font-bold text-primary mt-0.5 tracking-tight">{lotDetails.numero_lote}</p>
                ...
                <div className="mt-3 flex justify-center">
                    <Badge variant="outline" className={cn(
                        ...
                        {ESTADO_CONFIG[lotDetails.estado as keyof typeof ESTADO_CONFIG]?.label || lotDetails.estado}
                    </Badge>
                </div>
            </div>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                <TabsList className="w-full h-auto p-1 bg-muted/50 rounded-lg">
                    <TabsTrigger value="general" className="flex-1 text-xs gap-1.5 py-1.5 data-[state=active]:bg-card data-[state=active]:shadow-xs dark:data-[state=active]:bg-input/30 rounded-md">
                        <HugeiconsIcon icon={File02Icon} className="w-3.5 h-3.5" />
                        General
                    </TabsTrigger>
                    <TabsTrigger value="legal" className="flex-1 text-xs gap-1.5 py-1.5 data-[state=active]:bg-card data-[state=active]:shadow-xs dark:data-[state=active]:bg-input/30 rounded-md">
                        <HugeiconsIcon icon={RulerIcon} className="w-3.5 h-3.5" />
                        Legal
                    </TabsTrigger>
                    {showProcessTab && (
                        <TabsTrigger value="proceso" className="flex-1 text-xs gap-1.5 py-1.5 data-[state=active]:bg-card data-[state=active]:shadow-xs dark:data-[state=active]:bg-input/30 rounded-md">
                ...
                <TabsContent value="general" className="space-y-3 mt-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                                ...
                                {lotDetails.servidumbre_m2 ? (
                                    <div className="mt-1.5 pt-1.5 border-t border-sidebar-border/50 text-[10px] space-y-0.5">
                                        <div className="flex justify-between text-muted-foreground">
                                            ...
                                            <span>-{new Intl.NumberFormat('es-CL').format(lotDetails.servidumbre_m2)} m²</span>
                                        </div>
                                        <div className="flex justify-between font-medium text-foreground/80">
                                            <span>Sup. Neta útil:</span>
                                            <span>{new Intl.NumberFormat('es-CL').format(lotDetails.superficie_neta_m2 ?? 0)} m²</span>
                                        </div>
                                    </div>
                                ) : null}
                        ...
                        <Card className="shadow-xs border-sidebar-border/50">
                            ...
                            <CardContent className="p-3 pt-1">
                                <div className="text-lg font-bold text-foreground truncate">
                                    {lotDetails.precio
                                        ? new Intl.NumberFormat('es-CL', { style: 'currency', currency: 'CLP', maximumFractionDigits: 0 }).format(lotDetails.precio)
                                        : <span className="text-sm font-normal text-muted-foreground">Consulte</span>
                                    }
                                </div>
                                {lotDetails.valor_reserva ? (
                                    <div className="mt-1 flex items-center gap-1.5 text-[10px] text-muted-foreground">
                                        <span className="font-medium uppercase tracking-tight text-primary/60">Reserva:</span>
                                        <span className="font-medium text-foreground/80">
                                            {new Intl.NumberFormat('es-CL', { style: 'currency', currency: 'CLP', maximumFractionDigits: 0 }).format(lotDetails.valor_reserva)}
                                        </span>
                                    </div>
                                ) : null}
                            </CardContent>
                    ...
                    {lotDetails.observaciones && (
                        <div className="space-y-2 pt-2">
                            <p className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/50 ml-1">Observaciones</p>
                    ...
                    <TabsContent value="proceso" className="space-y-3 mt-3">
                        <Card className="border-primary/20 bg-primary/5 shadow-none">
                            <CardHeader className="p-4 pb-2">
                                <CardTitle className="text-[10px] uppercase tracking-widest text-primary/70">Progreso Venta</CardTitle>
                            </CardHeader>
                            <CardContent className="p-4 pt-2 space-y-4">
                                ...
                                <div className="flex flex-col gap-2">
                                    {lotDetails.etapa_proceso === 'espera_firma_reserva' && (
                                        <Button onClick={() => handleStageChange('reserva_firmada')} size="sm" variant="secondary" className="w-full">
                                            Confirmar Reserva Firmada
                                        </Button>
                                    )}
                                    {lotDetails.etapa_proceso === 'reserva_firmada' && (
                                        <Button onClick={() => handleStageChange('espera_firma_escritura')} size="sm" variant="secondary" className="w-full">
                                            Lista para Escritura
                                        </Button>
                                    )}
                                    {lotDetails.etapa_proceso === 'espera_firma_escritura' && (
                                        <Button onClick={() => handleStageChange('escritura_firmada')} size="sm" className="w-full bg-emerald-600 hover:bg-emerald-700 text-white">
