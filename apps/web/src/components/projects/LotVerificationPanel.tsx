...
import { RulerIcon, Shield02Icon, Copy01Icon, FloppyDiskIcon, Loading02Icon, Cancel01Icon, InformationSquareIcon } from '@hugeicons/core-free-icons'
...
const STATUS_CONFIG: Record<VerifiedStatus, { label: string; variant: 'default' | 'secondary' | 'destructive' | 'outline'; className: string }> = {
    draft: {
        label: 'Borrador',
        variant: 'outline',
        className: 'border-amber-300 text-amber-700 bg-amber-50 dark:border-amber-600 dark:text-amber-400 dark:bg-amber-500/20',
    },
    verified_exact: {
        label: 'Verificado (Coincide)',
        variant: 'default',
        className: 'bg-emerald-100 text-emerald-800 border-emerald-300 dark:bg-emerald-500/20 dark:text-emerald-400 dark:border-emerald-700',
    },
    verified_override: {
        label: 'Verificado (Override)',
        variant: 'default',
        className: 'bg-blue-100 text-blue-800 border-blue-300 dark:bg-blue-500/20 dark:text-blue-400 dark:border-blue-700',
    },
}
...
function boundariesToOfficial(calculated: BoundaryWithNeighbor[]): OfficialBoundaries {
    console.log('[DEBUG-PANEL] Mapeando calculatedBoundaries a Official:', JSON.stringify(calculated, null, 2))
    ...
    return calculated.map(seg => {
        ...
        if (seg.neighbors.length === 0) {
            ...
            colindaFinal = seg.neighbors.length === 2
                ? `${seg.neighbors[0].name} y ${seg.neighbors[1].name}`
                : seg.neighbors.length > 2
                    ? `${seg.neighbors.slice(0, -1).map(n => n.name).join(', ')} y ${seg.neighbors[seg.neighbors.length - 1].name}`
...
export function LotVerificationPanel({
    ...
    const [boundaries, setBoundaries] = useState<OfficialBoundaries>(() => {
        ...
        if (Array.isArray(lotDetails.boundaries_official) && lotDetails.boundaries_official.length > 0) {
    ...
    }, [projectId, lotDetails.id, areaOfficial, servidumbreOfficial, servidumbreAncho, boundaries, onLotUpdated])
                ...
                <div className="rounded-lg border border-border overflow-hidden">
                    ...
                    <div className="grid grid-cols-[1fr_70px_100px_60px] gap-0 items-center px-3 py-2.5 border-t border-border">
                        ...
                        <div className="px-1 relative flex items-center">
                            ...
                            />
                            <span className="absolute right-2 text-[9px] text-muted-foreground pointer-events-none">m²</span>
                        </div>
                    ...
                    <div className="space-y-2">
                        {boundaries.map((boundary, index) => {
                            ...
                            return (
                                <div
                                    key={index}
                                    className={`group rounded-lg border p-2.5 transition-colors ${isRoad
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div className="flex-1 flex items-center gap-1.5 min-w-0">
                                            <Input
                                                ...
                                                className={`h-7 px-1.5 text-xs font-semibold uppercase tracking-wide bg-transparent transition-colors ${isRoad
                                                ...
                                                <TooltipProvider delayDuration={200}>
                                                    <Tooltip>
                                                        <TooltipTrigger asChild>
                                                            <HugeiconsIcon icon={InformationSquareIcon} className="w-3.5 h-3.5 text-amber-500 shrink-0 cursor-help" />
                                                        </TooltipTrigger>
                                                        <TooltipContent side="top" className="text-[10px] max-w-50">
                                                            Este deslinde se suma al cálculo de servidumbre porque su colindancia menciona un camino.
                                                        </TooltipContent>
                                                    </Tooltip>
                                                </TooltipProvider>
                                            )}
                                        ...
                                        <InputGroup className={`w-24 shrink-0 h-7 ${isRoad ? 'border-amber-300 dark:border-amber-600' : ''}`}>
                                            <InputGroupInput
                                                type="number"
                                                step="0.01"
                                                value={boundary.distance ?? ''}
                                                onChange={(e) => handleBoundaryFieldChange(index, 'distance', e.target.value)}
                                                placeholder="0.00"
                                    ...
                                    <div className="mt-1.5">
                                        <Input
                                            ...
                                            className={`h-7 text-[11px] ${isRoad
                            ...
                            {!canVerify && (
                                <TooltipContent side="bottom" className="text-xs max-w-52">
                                    <div className="flex gap-1.5 items-start">
                                        <HugeiconsIcon icon={RulerIcon} className="w-3.5 h-3.5 text-amber-500 shrink-0 mt-0.5" />
                                        <span>
                                            Completa superficie y los 4 deslindes para verificar.
                                        </span>
                                    </div>
                                </TooltipContent>
                            )}
...
function DiffBadge({ diff }: { diff: number | null }) {
    ...
    return (
        <span className={`text - [10px] font - mono font - semibold text - right px - 1.5 py - 0.5 rounded ${color} `}>
            {sign}{diff.toFixed(1)}%
        </span>
    )
}
