...
export async function updateLotStage(
    ...
    try {
        ...
        if (fetchError || !lotRecord) {
            ...
            logger.error({ lotId, userId: user?.id, error: fetchError, found: !!lotRecord }, 'update_stage_not_found')
            return { success: false, error: `Error: Registro no encontrado. Lote: ${lotId.slice(0, 5)}...` }
        }
...
export async function directSale(
    ...
    const {
        cliente_nombre, cliente_run, cliente_direccion,
        cliente_estado_civil, cliente_ocupacion, cliente_email,
        cliente_telefono, fecha, notaria, valor_reserva
    } = validation.data
