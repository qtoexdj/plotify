...
export async function assignVendorsToProjectAction(
    ...
    try {
        ...
        if (!membership || membership.role !== 'admin') {
            return { success: false, error: 'Solo los administradores pueden asignar vendedores a proyectos' }
        }
                ...
                if (existingVendor) {
                    ...
                    const { data: newVendor, error: createError } = await serviceClient
                        ...
                        .insert({
                            ...
                            organization_id: organizationId,
                            owner_id: null,
                            nombre: profile ? `${profile.first_name || ''} ${profile.last_name || ''}`.trim() || profile.username : 'Nuevo Vendedor',
                    ...
                    if (createError) {
                        logger.error({ userId: item.userId, organizationId, error: createError }, 'assign_vendors_create_vendor_failed')
                        continue
                    }
        ...
        if (finalVendorIds.length === 0) {
            return { success: false, error: 'No se pudieron mapear los usuarios a registros de vendedores' }
        }
