...
export async function saveOfficialOverride(
    ...
    const { projectId, lotId, area_official_m2, servidumbre_m2, servidumbre_ancho_m, boundaries_official } = validation.data
    ...
    try {
        ...
        const { data: currentLot, error: fetchError } = await supabase
            .from('lots')
            .select('area_official_m2, perimeter_official_m, boundaries_official, verified_status, servidumbre_m2, servidumbre_ancho_m')
            .eq('id', lotId)
            .single()
...
export async function markLotVerified(
    ...
    const {
        projectId, lotId, verified_status,
        area_official_m2,
        boundaries_official, calculated_snapshot,
    } = validation.data
    ...
    try {
        ...
        return {
            success: true,
            message: verified_status === 'verified_exact'
