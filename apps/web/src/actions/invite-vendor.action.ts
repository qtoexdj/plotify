...
export async function inviteVendor(
    ...
    try {
        ...
        if (insertError) {
            logger.error({ organizationId, userId: inviteData.user.id, error: insertError }, 'invite_vendor_insert_member_failed')
        ...
        if (profileError) {
            logger.warn({ userId: inviteData.user.id, error: profileError }, 'invite_vendor_profile_create_warning')
        }
