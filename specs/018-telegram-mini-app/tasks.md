...
- [ ] T044 Validate all quickstart.md scenarios against live development sandbox. Evidence collected 2026-07-10: Supabase has 1 active bot with webhook and only 1 linked Telegram profile (requires admin + vendor); `miniapp.session_rejected` is audited once, but no `miniapp.session_issued` event exists. Infrastructure revalidated 2026-07-10: API and web respond locally; the public HTTPS Mini App at `https://efc5-201-215-26-80.ngrok-free.app/mini` and proxied `/api/v1/miniapp/health` return 200; `pnpm update:webhook` updated the active `@plotify_chat_bot` webhook and Telegram menu button, and Supabase persists that URL. Pending the six scenario outcomes from the required admin, vendor, and unlinked Telegram chats; record scenarios 0–5 before marking this task complete.

---

## Phase 9: Remediación post-revisión Supabase + Telegram

**Purpose**: Close critical SDD18 findings validated against the production-shaped Supabase schema and official Telegram Mini Apps behavior. Execute in order; do not mark a task complete until its stated verification passes.

- [x] T045 [US1] Add centralized Mini App project/lot authorization using the current session's org and `vendors.id`; apply it to map and lot-detail endpoints and add cross-tenant/cross-vendor negative tests in apps/api/api/v1/endpoints/miniapp.py and apps/api/tests/test_miniapp_endpoints.py. Verify: `pnpm test:api` (750 passed, 2 skipped).
- [x] T046 [US1] Normalize the canonical `org` deep-link parameter (with temporary `org_id` compatibility), correct menu and notification URLs to entity-specific routes, and omit `web_app` buttons when `TELEGRAM_MINI_APP_URL` is unset. Update frontend routing and Telegram notifier tests. Verify: `pnpm test:api` (752 passed, 2 skipped), `pnpm test:web` (846 passed), `pnpm typecheck:web`, and `pnpm build:web` passed; `pnpm --filter web lint` has only the pre-existing React Hook Form warning in `mini/reserva/page.tsx`.
- [x] T047 [US5] Pass `session.vendor_id` to reservation creation, make idempotency durable and atomic with a canonical Supabase migration, and add concurrent/restart-safe tests. Verify: manual SQL applied to Supabase; `pnpm verify:migrations` and `pnpm test:api` passed. The repository intentionally keeps canonical migrations in `packages/database/supabase/migrations`, whereas the Supabase CLI only inspects `supabase/migrations`; do not repair remote history for that expected layout difference. The migration uses `approval_requests(organization_id, vendor_id, idempotency_key)` UNIQUE and the reservation test simulates a post-restart duplicate conflict without a duplicate notification.
- [x] T048 [US3] Reuse canonical delivery listing/renewal services for Mini App documents; enforce recipient and organization scope and expose safe renewal of expired links. Verify: targeted endpoint tests (15 passed) and `pnpm test:api` passed with Supabase network access. The Mini App now delegates to `list_vendor_deliveries` and `renew_delivery_link`, which scope every operation to `recipient_user_id` and organization.
- [x] T049 [US1] Complete native Telegram SDK lifecycle (`ready`, `expand`, theme updates, BackButton) and add client tests. Verify: `pnpm test:web` (846 passed), `pnpm typecheck:web`, `pnpm build:web`, formatting, and web lint passed (the existing React Hook Form warning remains unrelated).
- [x] T050 [US1] Generate typed API contracts from FastAPI schemas, remove Mini App contract drift, and run contract/type checks. Verify: `pnpm contracts:generate`, formatting, `pnpm typecheck:web`, and `pnpm build:web` passed. Generated OpenAPI and TypeScript include the Mini App document listing/renewal, reservation, session, map, sales, and bandeja operations.
