...
- [ ] T001 Review the spec and implementation plan for bento layout unification.
...
- [ ] T002 Sync CodeGraph before implementation.
...
- [ ] T003 Create `PageShell` and `PageHeader`.
...
- [ ] T004 Create `BentoGrid` and `BentoPanel`.
...
- [ ] T005 Migrate `/projects` to shared page primitives.
...
- [ ] T006 Migrate `/vendors` to shared page primitives and remove title icon.
...
- [ ] T007 Migrate `/settings/workspace` to shared page primitives.
      Acceptance: page root no longer uses `p-6 max-w-4xl mx-auto`; workspace form
      aligns to the shared rail and occupies a bento column appropriate for forms.
      Verify: `pnpm --filter web lint`
...
- [ ] T008 Migrate `/dashboard` and `/clients`.
...
- [ ] T009 Migrate `/settings/profile` and `/documentos`.
      Acceptance: profile and document pages use the same page rail and title
      hierarchy; document quick-access modules fit the bento grid.
      Verify: `pnpm --filter web lint`
...
- [ ] T010 Update `design.md` with the canonical layout decision.
...
- [ ] T011 Run final web quality gates.
      Acceptance: lint, format check and production build pass.
