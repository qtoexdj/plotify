...
| D2  | El logo **se mantiene verde** `#16A34A` (decisión explícita del usuario, 2026-07-03, al resolver el pendiente de `brand/colors.md`); el carmesí queda exclusivo de la UI, sin mezclarse → tokenizar `--brand` igual (valor = verde) para dejar un solo punto de cambio a futuro | Migrar el logo a carmesí; acento verde en la UI; tinta neutra                                                                      |
