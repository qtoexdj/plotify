...
- **Paleta "Tinta nítida"**: monocromo de alto contraste con acento **carmesí** `#A93439` (claro) / `#C24444` (oscuro). El usuario confirmó (2026-07-03) que el logo de `brand/` **se mantiene verde** `#16A34A` — el verde sigue siendo el único color de marca del isotipo, y el carmesí es exclusivamente el acento de la UI; no se mezclan. Aun así el color del plano del isotipo queda **tokenizado** (`--brand`, con valor verde) para tener un solo punto de cambio si el usuario decide recolorear en el futuro.
...
5. **Given** el archivo `brand-mark-paths.tsx`, **When** el usuario quiera cambiar el color de marca en el futuro, **Then** basta modificar un único token (el verde `#16A34A` ya no está hardcodeado en componentes, aunque su valor vigente sigue siendo verde).
...
6. **Given** el header, **When** se inspecciona, **Then** no existe input de búsqueda ni toggle de tema; solo trigger de sidebar, breadcrumb/título, campana y avatar. El tema se cambia desde el menú del usuario (switch actual de NavUser) o vía ⌘K.
...
- **FR-004 (Marca en la app)**: el sidebar DEBE mostrar el mark real de `brand/` (componente derivado de `brand-mark-paths.tsx`); el color del plano en `brand-mark-paths.tsx` DEBE salir de `var(--brand)` (con fallback al verde `#16A34A` vigente) en vez de `#16A34A` literal; favicon/íconos/manifest DEBEN regenerarse con `brand/scripts/generate_assets.py` si el usuario recolorea los maestros en el futuro (tarea documentada, no bloqueante — hoy no aplica).
...
- **FR-008 (Header)**: el header del dashboard DEBE quedar en: SidebarTrigger, breadcrumb/título, NotificationBell y avatar. Sin input de búsqueda y sin toggle de tema. El cambio de tema vive donde está hoy — switch en el menú del usuario (NavUser, footer del sidebar) — más la acción "Cambiar tema" del command palette (FR-007); las páginas de auth conservan su ModeToggle flotante.
...
- El logo se mantiene verde; no hay recolor de los SVG maestros de `brand/` en esta iteración. Este feature solo deja el terreno tokenizado (`--brand`) y el proceso documentado por si se recolorea en el futuro.
