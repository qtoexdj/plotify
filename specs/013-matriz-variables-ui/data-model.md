...
- **Tipo**: `string | null` (enum lógico abajo).
...
- **Compatibilidad**: aditivo y nullable → no rompe consumidores existentes.
...
| `sale_gap` | la aporta la venta de cada lote | informativo ("se completa en la venta") / en borrador: rellena |
...
## Dos niveles (scope)
- **Proyecto (molde)**: inventario con `escritura_case_id = null`. Los `sale_gap` están ausentes → se muestran como "se completa en la venta".
- **Lote (borrador de venta)**: inventario del caso (`escritura_case_id`); los `sale_gap` vienen **rellenos** desde el snapshot/puente operacional → se muestran con valor y origen "desde la venta", de solo lectura en esta superficie.
