...
3. **Venta de un lote** → el vendedor registra la venta comercial (`LotReservationForm`: cliente + valor); el admin la aprueba; el hook `handle_sale_validated_for_escritura` crea el caso + snapshot y el **puente operacional** (`escritura_operational_bridge.py`) mapea lo comercial → `comprador.*`/`transaccion.*`/`lote.*`/`servidumbre.*`, copiando el molde aprobado a la escritura/caso del lote con los huecos ya rellenos. Este paso no exige una revisión legal lote a lote.
...
- **Riesgo**: dos paneles tienen capacidad fina — (a) **override manual de roles SII** por lote, (b) **Título** (cadena de adquisición, alertas, narrativa). Mitigación: (a) se preserva en `sii-lot-detail.tsx` reutilizando `legal-roles/[lotId]` (FR-013); (b) el grupo `titulo` usa un inspector de texto largo/estructurado para narrativa + alertas.

### D5 — Venta como relleno automático, no revisión lote a lote

La matriz por productor sirve para aprobar el molde del proyecto. La venta de cada lote rellena `comprador.*`, `transaccion.*`, `lote.*` y `servidumbre.*` desde el registro comercial al generar la escritura/caso del lote. Si se expone una vista posterior, debe ser de trazabilidad en modo lectura, no una segunda cola de revisión para el abogado.
...
- **Trazabilidad de la venta**: si se implementa una vista por lote, debe leer snapshot/puente y no editar comprador/precio (eso vive en la venta comercial). Esta vista es opcional y no bloquea el cierre del molde.
