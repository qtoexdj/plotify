-- ============================================================================
-- SDD019: Authz guards v2 — patrón canónico auth.role() = 'authenticated'
-- Fecha: 2026-08-13
-- Motivo: v1 (20260813150000) usaba coalesce(auth.role(),'') <> 'service_role',
-- que bloqueaba también a postgres/SQL directo (auth.role() es NULL sin JWT).
-- El nuevo guard solo intercepta el caso peligroso: usuario autenticado vía
-- PostgREST que no es admin de la organización. anon ya no tiene EXECUTE,
-- y service_role/postgres pasan sin cambios.
-- ============================================================================
...
DECLARE
    v_key TEXT;
BEGIN
    IF auth.role() = 'authenticated' AND NOT public.is_org_admin(p_org_id) THEN
