-- Agrega perimeter_official_m a la carga de valores oficiales.
-- `can_activate_project_sales` exige perimeter_official_m > 0 en todo lote
-- comercializable, así que sin este campo el proyecto nunca activa aunque los
-- lotes estén verificados. Se reemplaza la firma anterior.

drop function if exists public.set_lot_official_values_as_admin(uuid,uuid,numeric,numeric,numeric,numeric,text,jsonb,boolean,text);
...
create or replace function public.set_lot_official_values_as_admin(
  ...
  p_perimeter_official_m numeric default null,
...
begin
  ...
  update public.lots set
    ...
    perimeter_official_m    = coalesce(p_perimeter_official_m, perimeter_official_m),
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'next', jsonb_build_object(
        ...
        'perimeter_official_m', p_perimeter_official_m,
        'verified_status', case when p_verify then 'verified_override' else v_prev_status end))
  );
end;
$function$;

revoke execute on function public.set_lot_official_values_as_admin(uuid,uuid,numeric,numeric,numeric,numeric,text,numeric,jsonb,boolean,text) from anon, authenticated, public;
grant execute on function public.set_lot_official_values_as_admin(uuid,uuid,numeric,numeric,numeric,numeric,text,numeric,jsonb,boolean,text) to service_role;

comment on function public.set_lot_official_values_as_admin(uuid,uuid,numeric,numeric,numeric,numeric,text,numeric,jsonb,boolean,text) is
