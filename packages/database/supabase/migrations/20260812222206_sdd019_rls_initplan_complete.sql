-- Migration 20260812222206_sdd019_rls_initplan_complete.sql
-- Continuation of the RLS InitPlan optimization to wrap remaining auth.uid() calls with (select auth.uid())

-- ==============================================================================
-- 1. FUNCTIONS
-- ==============================================================================

-- 1. can_manage_project_files
create or replace function public.can_manage_project_files(project_id_text text)
returns boolean
language plpgsql
security definer
...
-- 2. can_read_project_files
create or replace function public.can_read_project_files(project_id_text text)
returns boolean
language plpgsql
security definer
set search_path to 'public'
...
begin
  ...
  return exists (
    select 1 from public.projects p
    left join public.organization_members om on p.organization_id = om.organization_id and om.user_id = (select auth.uid())
    left join public.vendor_projects vp on p.id = vp.project_id
    left join public.vendors v on vp.vendor_id = v.id and v.user_id = (select auth.uid())
    where p.id = p_uuid
      and (
...
-- 3. is_org_user
create or replace function public.is_org_user(org_id uuid)
returns boolean
language sql
security definer
set search_path to 'public'
...
BEGIN
  ...
  -- ONLY CHANGE: auth.uid() -> (select auth.uid())
...
-- 5. notification_events_vendor_select
drop policy if exists notification_events_vendor_select on public.notification_events;
create policy notification_events_vendor_select on public.notification_events
    for select
    using (recipient_id = (select auth.uid()));

-- 6. escritura_deliveries_vendor_select
drop policy if exists escritura_deliveries_vendor_select on public.escritura_deliveries;
...
-- 7-9. Avatar policies on storage.objects
  ...
  or (
    ...
    and (
      ...
      or exists (
        ...
        where v.id = vendedor_id and v.user_id = (select auth.uid()) and v.active
      )
    )
  )
)
